...
pub enum Error {
    ...
    DbConnectionClosed { message: String },
    DbMutexLockPoisoned { message: String },
    TableConnectionClosed { message: String },
    TableMutexLockPoisoned { message: String },
    // Rustler Errors
    RustlerBadArg,
...
impl Encoder for Error {
    fn encode<'a>(&self, env: Env<'a>) -> Term<'a> {
        let error_tuple = match self {
            ...
            Error::DbConnectionClosed { message } => {
                (atoms::db_connection_closed(), message.to_string())
            }
            Error::DbMutexLockPoisoned { message } => {
                (atoms::db_mutex_lock_poisoned(), message.to_string())
            }
            Error::TableConnectionClosed { message } => {
                (atoms::table_connection_closed(), message.to_string())
            }
            Error::TableMutexLockPoisoned { message } => {
                (atoms::table_mutex_lock_poisoned(), message.to_string())
            }
