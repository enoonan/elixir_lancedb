...
use crate::{atoms, error::{Error, Result}, runtime::get_runtime, rustler_arrow::schema::Schema};
...
impl Into<LanceNewColumnTransform> for NewColumnTransform {
  fn into(self) -> LanceNewColumnTransform {
      match self {
        NewColumnTransform::AllNulls(schema) => LanceNewColumnTransform::AllNulls(Arc::new(schema.into()))
      }
  }
}